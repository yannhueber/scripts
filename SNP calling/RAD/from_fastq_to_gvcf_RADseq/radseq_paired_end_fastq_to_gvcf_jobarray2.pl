...
#foreach my $key (sort {$a cmp $b} keys(%hash_file)) {
#	my $lol = $hash_file{$key};
#	my @hellyeah = split(m/-/,$lol);
#	my $oone = $hellyeah[0];
#	my $two = $hellyeah[1];
#	print "ola, quetal : $key gave me $oone and $two\n";
#}
...
foreach my $file ( ($forward,$reverse) ) {
	...
	#Cutadapt
	my $rm_adapter = "$cutadapt_path/cutadapt -b AGATCGGAAGAGC -O 7 -m 30 --quality-base=64 -q 20,20 $file | gzip > $filenm_without_ext.cutadapt.fastq.gz";
	print ("cutadapt on $file\n");
	system("$rm_adapter");

	#fastqc again
	$fastqc = "$fastqc_path/fastqc -j $java_path/java $filenm_without_ext.cutadapt.fastq.gz";
...
#bwa
my $bwa_mapping = "$bwa_path/bwa mem $reference $forward_without_ext.cutadapt.cleaned.fastq $reverse_without_ext.cutadapt.cleaned.fastq > $filenm_root.sam";
...
my $add_rg = "$java_path/java -Xmx4g -jar $picard_path/picard.jar AddOrReplaceReadGroups I=$filenm_root.sam O=$filenm_root.RG.sorted.bam SO=coordinate RGLB=$cultivar RGPL=illumina RGPU=run RGSM=$condition RGID=$condition";
...
my $realigner_target = "$java_path/java -Xmx4g -jar $gatk_path/GenomeAnalysisTK.jar -T RealignerTargetCreator -R $reference -I $filenm_root.RG.sorted.bam --fix_misencoded_quality_scores -o $filenm_root.forIndelRealigner.intervals";
print ("Create RealignTargetCreator intervals with $filenm_root.RG.sorted.bam...\n");
system("$realigner_target");

#IndelRealigner
my $indelrealigner = "$java_path/java -Xmx4g -jar $gatk_path/GenomeAnalysisTK.jar -T IndelRealigner -R $reference -I $filenm_root.RG.sorted.bam -targetIntervals $filenm_root.forIndelRealigner.intervals --fix_misencoded_quality_scores -o $filenm_root.indelrealigned.RG.sorted.bam";
print ("IndelRealigning on file $filenm_root.RG.sorted.bam...\n");
system("$indelrealigner");

#HaplotypeCaller (pour BaseRecalibrator)
#my $haplotype_caller = "$java_path/java -Xmx4g -jar $gatk_path/GenomeAnalysisTK.jar -T HaplotypeCaller -R $reference -I $filenm_root.indelrealigned.RG.sorted.bam -o $filenm_root.indelrealigned.RG.sorted.HC.vcf";
...
my $haplotype_caller_gvcf = "$java_path/java -Xmx4g -jar $gatk_path/GenomeAnalysisTK.jar -T HaplotypeCaller -R $reference -I $filenm_root.indelrealigned.RG.sorted.bam --emitRefConfidence GVCF --variant_index_type LINEAR --variant_index_parameter 128000 -o $filenm_root.indelrealigned.RG.sorted.HC.gvcf";
...
#my $variant_annotation = "$java_path/java -Xmx4g -jar $gatk_path/GenomeAnalysisTK.jar -T VariantAnnotator -R $reference -V $filenm_root.indelrealigned.RG.sorted.HC.vcf -I $filenm_root.indelrealigned.RG.sorted.bam -A MappingQualityZero -o $filenm_root.indelrealigned.RG.sorted.HC.ann.vcf";
...
#my $select_variant = "$java_path/java -Xmx4g -jar $gatk_path/GenomeAnalysisTK.jar -T SelectVariants -R $reference -V $filenm_root.indelrealigned.RG.sorted.HC.ann.VF.vcf -select 'vc.isNotFiltered()' -selectType SNP -o $filenm_root.indelrealigned.RG.sorted.HC.ann.VF.filtered.vcf";
...
#BaseRecalibrator
#my $base_recalibrator = "$java_path/java -Xmx4g -jar $gatk_path/GenomeAnalysisTK.jar -T BaseRecalibrator -R $reference -I $filenm_root.indelrealigned.RG.sorted.bam --knownSites $filenm_root.indelrealigned.RG.sorted.HC.ann.VF.filtered.vcf -o $filenm_root.recal_data.table";
